 ...
 * @returns {MosaicPath}
 */
function getPathFromNode(node, tree, path) {
    if (path === void 0) { path = []; }
    var currentNode = tree;
    if (isParent(currentNode)) {
        var first = getPathFromNode(node, currentNode.first, path.concat(['first']));
        if (first.length) {
            return first;
        }
        var second = getPathFromNode(node, currentNode.second, path.concat(['second']));
        if (second.length) {
            return second;
        }
    }
...
exports.getPathFromNode = getPathFromNode;
