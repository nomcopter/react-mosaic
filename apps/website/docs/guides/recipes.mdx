...
`onChange` receives only the new tree. To find out what disappeared, compare
leaves before and after:

```tsx
import { getLeaves, MosaicNode } from 'react-mosaic-component';

const handleChange = (next: MosaicNode<string> | null) => {
  const before = new Set(getLeaves(tree));
  const after = new Set(getLeaves(next));
  const removed = [...before].filter((id) => !after.has(id));
  if (removed.length > 0) {
    onPanelsRemoved(removed);
  }
  setTree(next);
};
```

If you only care about the remove button, pass your own `RemoveButton` in
`toolbarControls`. Its `onClick` runs right after the panel is removed, and
you already know the `id` from `renderTile`:
...
renderTile={(id, path) => (
  <MosaicWindow
    path={path}
    title={id}
    toolbarControls={<RemoveButton onClick={() => onPanelRemoved(id)} />}
  >
    ...
  </MosaicWindow>
)}
