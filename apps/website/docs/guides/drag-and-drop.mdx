...
function PaletteItem({ panelKey, onDrop }: {
  ...
  return <div ref={drag}>{panelKey}</div>;
}
...
out of the layout, firing `onChange` and `onRelease` as usual. Return nothing
and the window snaps back to where it was.
...
function Sidebar({ onPanelDropped }: { onPanelDropped: (key: string) => void }) {
  ...
  return (
    <aside ref={drop} className={isOver ? 'sidebar drop-here' : 'sidebar'}>
